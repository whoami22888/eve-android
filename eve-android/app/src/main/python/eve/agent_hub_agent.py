...
class AgentHubAgent:
    ...
    def _emit(self, method: str, *args):
        try:
            from java import jclass; getattr(jclass("com.eve.agent.EveKotlinBridge"), method)(*args)
    ...
    def _apply_coder_files(self, workspace, result):
        changed=[]
        for item in result.get("files", []) if isinstance(result.get("files", []), list) else []:
            if isinstance(item, dict) and item.get("path") and isinstance(item.get("content"), str): workspace.write(str(item["path"]), item["content"]); changed.append(str(item["path"]))
        return changed
    def _redact(self, text):
        ...
        return redacted
    def _run_tests(self, workspace, commands, task_id=""):
        results=[]
    ...
    def _control(self, task):
        ...
        for task_id in targets:
            ...
            if not target_task and command=="retry" and task_id not in {"","all"} and task.params.get("task"):
                target_task=Task("agent_hub","agent_hub",{"task":str(task.params.get("task")),"project":str(task.params.get("project","default"))},task_id)
                with self._state_lock:
                    self._task_objects[task_id]=target_task; self._task_specs[task_id]={"prompt":str(task.params.get("task")),"project":str(task.params.get("project","default"))}; self._task_stage[task_id]=str(task.params.get("stage","Plan")).lower()
