...
class AgentHubAgent:
    ...
    def _emit_stage(self, task_id: str, stage: str, status: str, progress: int, message: str = "") -> None:
        ...
        try:
            ...
            jclass("com.eve.agent.EveKotlinBridge").onPipelineStage(task_id, _STAGE_LABELS.get(stage, stage), status, int(progress), message)
        except Exception:
            pass
    ...
    def _run_tests(self, workspace: ProjectWorkspace, commands: List[str], task_id: str = "") -> List[Dict]:
        ...
        for command in commands[:3]:
            ...
            if self._cancelled(task_id):
                results.append({"command": command, "status": "cancelled"}); break
            if command not in _ALLOWED_TEST_COMMANDS:
                results.append({"command": command, "status": "skipped", "reason": "not allowlisted"}); continue
            proc = None
            try:
    ...
    def _control(self, task: Task) -> None:
        target = str(task.params.get("task_id", "")).strip().lower()
        command = str(task.params.get("command", "")).strip().lower()
        with self._state_lock:
            targets = list(self._task_objects) if target == "all" else [target]
        matched = False
        for task_id in targets:
            with self._state_lock:
                pause = self._pause_events.get(task_id); cancel = self._cancel_events.get(task_id); target_task = self._task_objects.get(task_id)
            ...
            if command == "pause" and pause: pause.set(); task.result = f"Paused {task_id}"
                ...
                with self._state_lock:
                    self._cancel_events[task_id] = threading.Event(); self._pause_events[task_id] = threading.Event()
                target_task.status = "running"
                self._executor.submit(self._execute_pipeline, target_task, self._task_specs.get(task_id, {}), self._task_stage.get(task_id, "planner"))
                task.result = f"Retry started for {task_id}"
            else:
                matched = False
        if not matched:
            task.status = "failed"; task.error = f"Unknown or unavailable Agent Hub control: {command}"; return
        task.status = "completed"

    def assign_task(self, task: Task) -> None:
        if task.action == "agent_hub_control":
            self._control(task); return
        prompt = str(task.params.get("task", "")).strip()
        project = str(task.params.get("project", "default")).strip() or "default"
        if not prompt:
            task.status = "failed"; task.error = "Agent Hub task is empty"; return
        try:
            ProjectWorkspace(self.data_dir, project)
        except WorkspaceError as exc:
            task.status = "failed"; task.error = str(exc); return
        task.status = "queued"
        with self._state_lock:
            self._cancel_events[task.id] = threading.Event(); self._pause_events[task.id] = threading.Event()
            self._task_objects[task.id] = task; self._task_specs[task.id] = {"prompt": prompt, "project": project}
    ...
    def _execute_pipeline(self, task: Task, spec: Dict, start_stage: str = "planner") -> None:
        prompt = str(spec.get("prompt", "")).strip(); project = str(spec.get("project", "default")).strip() or "default"
        task.status = "running"
        ...
        try:
            ...
            if start <= 2:
                ...
                context["changed"] = self._apply_coder_files(workspace, context["coder"]); snapshot = workspace.snapshot()
            if start <= 3:
            ...
            task.result = self._redact(json.dumps({"project": project, "stages": context}, ensure_ascii=False)); task.status = "completed"; task.completed_at = time.time()
            ...
            task.status = "failed"; task.error = self._redact(str(exc))
            ...
            with self._state_lock:
                self._cancel_events.pop(task.id, None); self._pause_events.pop(task.id, None); self._active_processes.pop(task.id, None)
