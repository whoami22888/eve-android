...
class GeminiProvider:
    ...
    def complete(self, system: str, user: str, temperature: float = 0.1) -> str:
        ...
        url = self.config.base_url.rstrip("/") + f"/v1beta/models/{self.config.model}:generateContent"
        data = _request_json(url + "?key=" + urllib.parse.quote(self.config.api_key, safe=""), payload, {}, self.config.timeout_seconds)
        try:
